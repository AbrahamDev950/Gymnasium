...
public class AsistenciasController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<ActionResult<AsistenciaResponse?>> RegistrarAsistencia(AsistenciaRequest request)
    {
        if (!ModelState.IsValid)
            return BadRequest(ModelState);

        var respuesta = await _asistenciaService.RegistrarAsistencia(request.SocioId);
